...
class BendLine {
  constructor() {
    ...
    for (let i = 0; i < this.numBend; i++) {
      this.aryRParameter[i] = [this.r / 4 * 1**i, random(8*PI), 2*PI / random(50, 200)];//[max, init, speed]
    }
    this.aryinitAngParameter = [];
    for (let i = 0; i < this.numBend; i++) {
      this.aryinitAngParameter[i] = [PI, random(PI), PI / random(100, 500)];//[max, init, speed]
    }
    this.aryAngStep = [];
    for (let i = 0; i < this.numBend; i++) {
      this.aryAngStep[i] = PI / this.numPoints * int(random(2, i));
    }
    this.numCycle = 2;
  }
  update() {
  ...
  draw() {
    ...
    for (let i = 0; i < this.newAryXy.length-1; i++) {
      noFill();
  strokeWeight(20)
      // rect(this.newAryXy[i].y, this.newAryXy[i].x, 200, 100);
      // rect(this.newAryXy[i].x, this.newAryXy[i].y, 100, 200);

      // ellipse(this.newAryXy[i].y, this.newAryXy[i].x, 20, 20)
      strokeWeight(10)
