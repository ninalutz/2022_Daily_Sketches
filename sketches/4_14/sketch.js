...
class BendLine {
  constructor(centXy, r, col) {
    ...
    for (let i = 0; i < this.numBend; i++) {
      this.aryRParameter[i] = [this.r / 30 * 0.8**i, random(PI), 2*PI / random(100, 200)];//[max, init, speed]
    }
    ...
    this.aryAngStep = [];
    for (let i = 0; i < this.numBend; i++) {
      this.aryAngStep[i] = 2*PI / this.numPoints * int(random(1, 2));
    }
    this.numCycle = 12;
  }
  update() {
  ...
  draw() {
    ...
    for (let i = 0; i < this.newAryXy.length; i++) {
      ellipse(this.newAryXy[i].x, this.newAryXy[i].y, 5, 5);
    }
    endShape(CLOSE);
  }
}
