...
class BendLine {
  constructor() {
    ...
    for (let i = 0; i < this.numBend; i++) {
      this.aryRParameter[i] = [this.r / 8 * 1**i, random(8*PI), 2*PI / random(50, 200)];//[max, init, speed]
    }
    this.aryinitAngParameter = [];
    for (let i = 0; i < this.numBend; i++) {
      this.aryinitAngParameter[i] = [PI, random(10*PI), 2*PI / random(200, 500)];//[max, init, speed]
    }
    this.aryAngStep = [];
    for (let i = 0; i < this.numBend; i++) {
  ...
  draw() {
    ...
    for (let i = 0; i < this.newAryXy.length-1; i++) {
      noFill();
  strokeWeight(5)
      rect(this.newAryXy[i].x, this.newAryXy[i].y, 50, 50);
        strokeWeight(2)
...
function bend(aryXy, r, initAng, angStep, numCycle) {
  ...
  for (let j = 0; j < numCycle; j++) {
    ...
    for (let i = 0; i < aryXy2.length; i++) {
      ...
      xy_1_2.rotate(-PI/2).setMag(r * atan(ang*2));
