...
class BendLine {
  constructor() {
    ...
    for (let i = 0; i < this.numPoints; i++) {
      let ang = PI / this.numPoints * i;
      this.aryXy[i] = createVector(this.r * cos(ang), this.r * sin(ang));
    }
    
    this.numBend = 10;
    this.aryRParameter = [];
    for (let i = 0; i < this.numBend; i++) {
      this.aryRParameter[i] = [this.r / 30 * 1**i, random(8*PI), 2*PI / random(100, 500)];//[max, init, speed]
    }
    this.aryinitAngParameter = [];
    for (let i = 0; i < this.numBend; i++) {
      this.aryinitAngParameter[i] = [2*PI, random(2*PI), 2*PI / random(200, 500)];//[max, init, speed]
    }
    this.aryAngStep = [];
    for (let i = 0; i < this.numBend; i++) {
      this.aryAngStep[i] = PI / this.numPoints * int(random(3, 8 + i));
    }
    this.numCycle = 2;
  }
  update() {
